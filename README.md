...
- **Fix previous spelling** (`Alt+F`): Automatically finds and fixes the most recent misspelled word before your cursor by accepting the first suggestion from the native spellchecker (if your cursor is in the middle of a word, it will check it as well).
...
The plugin scans backwards from your cursor position to find the first misspelled word in the current paragraph. It uses the native browser/Electron spellchecker API to detect misspellings and get suggestions, then automatically replaces the word with the first suggestion.
